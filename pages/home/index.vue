        ...
        <div class=" p-1 rounded-lg mb-6 border border-gray-200">
              ...
              <div class="cursor-pointer flex justify-between items-center">
                <div class="">
                  <h1 class="text-xl md:text-2xl ">
                    สวัสดี, คุณ <span class="text-sky-600 font-bold">{{ nameuser || "กำลังโหลด" }}</span>
                  </h1>
                </div>
          ...
          <div class="flex flex-col items-center m-[20px] text-black">
            ...
            <div class="text-3xl md:text-4xl font-bold m-[10px]">
              <span class="text-sky-600">{{ formatNumber(amount.value) }}</span> บาท
            </div>
          </div>
        </div>
      </div>
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-w-4xl mx-auto">
        <NuxtLink v-for="(item, index) in filteredMenuItems" :key="item.id"
          ...
          <div :class="[
            ...
            'min-h-[140px]',
            !isDisabled(item.title) && ' border-gray-200 hover:border-sky-300 hover:text-sky-600',
          ]">
            <div :class="[
              'rounded-xl ',
      ...
      <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-2 max-w-4xl mx-auto">
        <div class="bg-white rounded-lg p-4 md:p-6 mt-[25px] border border-gray-200">
          ...
          <div v-if="checkData && checkData.length > 0">
            <div v-for="(transaction, index) in displayedTransactions" :key="transaction.account_transition_id"
            ...
            <div v-if="checkData.length > 5" class="text-center mt-4">
              <button @click="showAllTransactions = !showAllTransactions"
