<template>
  <div class="min-h-screen flex items-start justify-center ">
    <div class="w-full max-w-sm md:max-w-md lg:max-w-lg bg-white rounded-xl h-full p-2">
      <!-- Header -->
      <div class="flex items-center space-x-2 mb-4">
        <div class="">
          <HooksGetTypeName class="text-xl md:text-2xl  font-semibold text-sky-600" />
        </div>
      </div>
...
const submitForm = async () => {
  try {
    ...
    showAlert(`เพิ่มเรียบร้อยแล้ว`, "ท่านสามารถเช็คได้โดยการกด <br/> กลับไปที่หน้าหมวดหมู่", `success`);
    fetchType();
  } catch (error) {
