...
const handleUpdate = async ({
  ...
  try {
    ...
    const response = await $fetch(
      "http://localhost:5000/api/transition_select_expense",
      {
        method: "POST",
        body: formData.value,
      }
...
const fetchMenuGroupData = async () => {
  try {
    const menuGroup_result = await $fetch(
      "http://localhost:5000/api/getMenuGroup_expense"
    );
...
const fetchTransitions = async () => {
  try {
    transition.value = await $fetch("http://localhost:5000/api/get_expense_transition");
