<template>
  <div class="max-w-3xl mx-auto space-y-4 rounded-lg">
    ...
    <div v-else class="space-y-2 mx-2">
      ...
      >
        <HandCoins class="w-12 h-12 text-green-500"/>
        <p>บันทึกรายการรายได้</p>
      </div>
      <DataIncomeComponent />
      <hr class="border-2 border-gray-200 mx-2" />
      <div div v-if="transition.length > 0">
...
const fetchTransitions = async () => {
  try {
    const response = await $fetch(
      "http://localhost:5000/api/get_income_transition"
    );
...
const deleteTransection = async (id, value) => {
  ...
  console.log(id);
  try {
    await $fetch(`http://localhost:5000/api/delete_transition_income/${id}`, {
      method: "PUT",
